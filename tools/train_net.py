...
def train_epoch(
    ...
    for cur_iter, (inputs, labels, _, meta) in enumerate(train_loader):
        ...
        # train_meter.iter_tic()
            ...
            binary_preds = torch.sigmoid(preds).round()
            # print("Labels shape:", labels.shape)
            # print("Binary_preds shape:", binary_preds.shape)
            
            # Compute the binary accuracy.
            binary_preds = binary_preds.argmax(dim=1)
            acc = accuracy_score(labels.detach().cpu().numpy(), binary_preds.detach().cpu().numpy())
    ...
    for cur_iter, (inputs, labels, _, meta) in enumerate(val_loader):
        ...
        if cfg.DETECTION.ENABLE:
            ...
            if cfg.DATA.MULTI_LABEL:
                ...
                binary_preds = binary_preds.argmax(dim=1)
                # Compute the metrics.
                acc, prec, rec, f1 = accuracy_score(labels.detach().cpu().numpy(), binary_preds.detach().cpu().numpy()), \
                                    precision_score(labels.detach().cpu().numpy(), binary_preds.detach().cpu().numpy()), \
                                    recall_score(labels.detach().cpu().numpy(), binary_preds.detach().cpu().numpy()), \
                                    f1_score(labels.detach().cpu().numpy(), binary_preds.detach().cpu().numpy())

                if cfg.NUM_GPUS > 1:
                    acc, prec, rec, f1 = du.all_reduce([acc, prec, rec, f1])

                # Copy the metrics from GPU to CPU (sync point).
                acc, prec, rec, f1 = acc.item(), prec.item(), rec.item(), f1.item()
